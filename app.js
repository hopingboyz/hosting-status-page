...
db.run(`ALTER TABLE users ADD COLUMN updated_at TEXT DEFAULT NULL`, (err) => {
...
async function initDB() {
    return new Promise((resolve, reject) => {
        db.serialize(() => {
            // Tables (Enhanced with new fields: cpu_cores, groups, audit_logs; indexes)
            const tables = [
...
wss.on('connection', (ws, req) => {
    ...
    // Fixed: Extract session ID from cookie properly
...
app.get('/status', (req, res) => {
    if (settingsCache.enable_public_status !== 'true') {
        return res.status(404).render('error', {
            message: 'Public status page is disabled.'
        });
    }
    db.all(`
        SELECT n.*, ns.*
        FROM nodes n
        LEFT JOIN node_stats ns ON ns.node_id = n.id AND ns.id = (
            SELECT id FROM node_stats WHERE node_id = n.id ORDER BY timestamp DESC LIMIT 1
        )
        WHERE n.status_page_visible = 1 AND n.public = 1
        ORDER BY n.name
    `, (err, nodes) => {
        if (err) {
            console.error("Error fetching nodes for status page:", err);
            nodes = [];
        }
...
app.get('/admin/settings', requireAuth, requireAdmin, (req, res) => {
  db.all("SELECT * FROM settings ORDER BY category, key", (err, rows) => {
      ...
      // Sync cache with DB for consistency
      rows.forEach(row => {
          settingsCache[row.key] = row.value;
      });
      log(`Synced ${rows.length} settings from DB`);
      // Convert DB rows to key-value object
...
app.get('/admin/export/:type', requireAuth, requireAdmin, (req, res) => {
    const { type } = req.params; // 'nodes', 'stats', 'users', 'alerts'
    const validTypes = ['nodes', 'stats', 'users', 'alerts'];
...
app.get('/admin/nodes/view/:id', requireAuth, (req, res) => {
    ...
    db.get("SELECT * FROM nodes WHERE id = ?", [nodeId], (err, node) => {
        ...
        db.all("SELECT * FROM node_stats WHERE node_id = ? ORDER BY timestamp DESC LIMIT 100", [nodeId], (err, stats) => {
            if (err) {
                console.error("Error fetching node stats:", err);
                stats = [];
            }
            db.all("SELECT * FROM alerts WHERE node_id = ? ORDER BY created_at DESC LIMIT 50", [nodeId], (err, alerts) => {
                if (err) {
                    console.error("Error fetching alerts:", err);
                    alerts = [];
                }
    ...
    db.get("SELECT id, name FROM nodes WHERE api_key = ?", [api_key], (err, node) => {
        ...
        db.run(
            `INSERT INTO node_stats (node_id, cpu_usage, cpu_cores, memory_usage, memory_total, memory_free,
             disk_usage, disk_total, disk_free, network_rx, network_tx, uptime, load_average, cpu_temp, processes, os_version)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                node.id,
    ...
    return new Promise((resolve) => {
        ...
        db.get(`SELECT id FROM alerts WHERE node_id = ? AND type = ? AND resolved = 0 ORDER BY created_at DESC LIMIT 1`, [nodeId, type], (err, existing) => {
            ...
            db.run(
                ...
                function(err) {
                    if (!err) {
                        broadcast({ type: 'new_alert', alert: { id: this.lastID, node_id: nodeId, type, message, severity } });
                    }
                    resolve();
                }
            );
